...
// All identities are treated as guests; token (if present) is used only to
// derive a stable per-browser id so the same browser keeps the same user_id.
handlers.auth = (ws, data) => {
...
handlers.create_session = (ws, data) => {
  ...
  // Game-agnostic share URL — root join page resolves the game from the code
  const shareUrl = `/join.html?code=${code}`;
